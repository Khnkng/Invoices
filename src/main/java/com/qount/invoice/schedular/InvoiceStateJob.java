	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			if (conn != null) {
				...
				// < ?) AS ids);";
				String query = "UPDATE invoice SET state = 'past_due' WHERE id IN ( SELECT id  FROM (SELECT id FROM invoice WHERE state !='paid' AND state IN ('sent','partially_paid') AND due_date < ?) AS ids);";

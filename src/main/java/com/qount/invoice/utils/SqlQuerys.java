...
public class SqlQuerys {
	...
	public final class Invoice {
		public static final String INSERT_QRY = "insert into `invoice` (`id`,`user_id`,`company_id`,`amount`,`currency`,`description`,`objectives`,`last_updated_by`,`last_updated_at`,`state`,`invoice_date`,`acceptance_date`,`acceptance_final_date`,`notes`,`discount`,`deposit_amount`,`processing_fees`,`remainder_json`,`remainder_mail_json`,`is_recurring`,`recurring_frequency`,`recurring_frequency_value`,`recurring_start_date`,`recurring_end_date`,`is_mails_automated`,`is_cc_current_user`,`payment_spring_customer_id`,`po_number`,`document_id`,`amount_due`,`payment_date`,`customer_id`,`sub_totoal`,`amount_by_date`) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		public static final String UPDATE_QRY = "UPDATE `invoice` SET  `user_id` =?,`company_id` =?,`amount` =?,`currency` =?,`description` =?,`objectives` =?,`last_updated_by` =?,`last_updated_at` =?,`state` =?,`invoice_date` =?,`acceptance_date` =?,`acceptance_final_date` =?,`notes` =?,`discount` =?,`deposit_amount` =?,`processing_fees` =?,`remainder_json` =?,`remainder_mail_json` =?,`is_recurring` =?,`recurring_frequency` =?,`recurring_frequency_value` =?,`recurring_start_date` =?,`recurring_end_date` =?,`is_mails_automated` =?,`is_cc_current_user` =?,`payment_spring_customer_id` =?,`po_number` =?,`document_id` =?,`amount_due` =?,`payment_date` =?,`sub_totoal` =?,`customer_id` =?, `amount_by_date` = ? WHERE `id` =?";
		public final static String DELETE_QRY = "DELETE FROM invoice WHERE `id`=?;";
		public final static String GET_QRY = "SELECT i.`id`,i.`user_id`,i.`company_id`,i.`amount`,i.`currency`,i.`description`,i.`objectives`,i.`last_updated_by`,i.`last_updated_at`,i.`customer_id`,i.`state`, i.`invoice_date`,i.`acceptance_date`,i.`acceptance_final_date`,i.`notes`,i.`discount`,i.`deposit_amount`,i.`processing_fees`,i.`remainder_json`, i.`remainder_mail_json`,i.`is_recurring`,i.`recurring_frequency`,i.`recurring_frequency_value`,i.`recurring_start_date`,i.`recurring_end_date`,i.`is_mails_automated`,i.`is_cc_current_user`,i.`payment_spring_customer_id`,i.`po_number`,i.`document_id`,i.`amount_due`,i.`payment_date`,i.`sub_totoal`,i.`amount_by_date`,c.`code`,c.`name`,c.`html_symbol`, il.`id` AS `ilid`,il.`item_id` AS `il_item_id`,il.`invoice_id`,il.`description` `il_description`,il.`objectives` `il_objectives`,il.`amount` `il_amount`,il.`currency` `il_currency`,il.`last_updated_by` `il_last_updated_by`,il.`last_updated_at` `il_last_updated_at`,il.`quantity` `il_quantity`,il.`price` `il_price`,il.`notes` `il_notes`,il.`coa_id` `il_coa_id`,cl.`code` `il_code`,cl.`name` `il_name`,cl.`html_symbol` `il_html_symbol`, ilt.`invoice_line_id` `ilt_invoice_line_id`,ilt.`tax_id` `ilt_tax_id`,ilt.`tax_rate` `ilt_tax_rate` FROM `invoice` i LEFT JOIN `currencies` c ON i.currency=c.code LEFT JOIN `invoice_lines` il ON i.id=il.invoice_id LEFT JOIN `currencies` cl ON il.currency=cl.code LEFT JOIN `invoice_line_taxes` ilt ON il.id=ilt.invoice_line_id  WHERE i.id  =?;";
		public final static String GET_INVOICES_LIST_QRY = "SELECT `id`,`user_id`,`company_id`,`amount`,`currency`,`description`,`objectives`,`last_updated_by`,`last_updated_at`,`state`,`invoice_date`,`acceptance_date`,`acceptance_final_date`,`notes`,`discount`,`deposit_amount`,`processing_fees`,`remainder_json`,`remainder_mail_json`,`is_recurring`,`recurring_frequency`,`recurring_frequency_value`,`recurring_start_date`,`recurring_end_date`,`is_mails_automated`,`is_cc_current_user`,`payment_spring_customer_id`,`number`,`po_number`,`document_id`,`amount_due`,`payment_date`,`sub_totoal`,`customer_id`,`amount_by_date` FROM invoice WHERE `user_id`=?;";
	...
	public final class InvoiceLine {
		public final static String INSERT_QRY = "INSERT INTO `invoice_lines` (`id`,`invoice_id`,`description`,`objectives`,`amount`,`currency`,`last_updated_by`,`last_updated_at`,`quantity`,`price`,`notes`,`item_id`,`coa_id`) values (?,?,?,?,?,?,?,?,?,?,?,?,?);";
		public final static String UPDATE_QRY = "update `invoice_lines` SET `invoice_id`=?,`description`=?,`objectives`=?,`amount`=?,`currency`=?,`last_updated_by`=?,`last_updated_at`=?,`quantity`=?,`price`=?,`notes`=?, `item_id`=?, `coa_id` = ? where `id`=?";
		public final static String GET_LINES_QRY = "SELECT `id`,`invoice_id`,`description`,`objectives`,`amount`,`currency`,`last_updated_by`,`last_updated_at`,`quantity`,`price`,`notes`,`item_id`,`coa_id` FROM invoice_lines WHERE `invoice_id` = ?;";

...
public class SqlQuerys {
	...
	public final class Payments {
		...
		public static final String RETRIEVE_INVOICES_BY_PAYMENTID = "SELECT `invoice`.`id` AS invoice_id,`invoice`.`number`,`invoice`.`invoice_date`,`invoice`.`due_date`,`invoice`.`amount`,`invoice`.`amount_due`,`invoice`.`state`, invoice_payments_lines.id AS payment_lineID,invoice_payments.id AS paymentID  FROM invoice_payments LEFT JOIN invoice_payments_lines ON invoice_payments.id = invoice_payments_lines.`payment_id` LEFT JOIN invoice ON invoice_payments_lines.`invoice_id` = invoice.`id` WHERE `invoice_payments`.`id` = ? AND invoice_payments_lines.`amount`>0";
	}

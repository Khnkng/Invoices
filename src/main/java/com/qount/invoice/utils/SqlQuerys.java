...
public class SqlQuerys {
	...
	public final class Invoice {
		public static final String INSERT_QRY = "INSERT INTO invoice (`journal_job_id`, `late_fee_applied`, `late_fee_id`, `late_fee_amount`, `attachments_metadata`, `id`, `user_id`, `company_id`, `customer_id`, `amount`, `currency`, `description`, `objectives`, `last_updated_by`, `last_updated_at`, `state`, `invoice_date`, `notes`, `discount`, `deposit_amount`, `processing_fees`, `number`, `document_id`, `amount_due`, `due_date`, `sub_totoal`, `amount_by_date`, `created_at`, `amount_paid`, `term`, `created_at_millis`, `recepients_mails`, `plan_id`, `is_recurring`, `payment_options`, `email_state`, `send_to`,`payment_method`,`tax_amount`,`proposal_id`,`remainder_job_id`,`remainder_name` ) VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		public static final String UPDATE_QRY = "UPDATE invoice SET `journal_job_id` = ?, `late_fee_applied` = ?, `late_fee_id` = ?, `late_fee_amount` = ?, `attachments_metadata`=?,`remainder_job_id` = ?, `remainder_name` = ?, `user_id` = ?, `company_id` = ?, `customer_id` = ?, `amount` = ?, `currency` = ?, `description` = ?, `objectives` = ?, `last_updated_by` = ?, `last_updated_at` = ?, `state` = ?, `invoice_date` = ?, `notes` = ?, `discount` = ?, `deposit_amount` = ?, `processing_fees` = ?, `number` = ?, `document_id` = ?, `amount_due` = ?, `due_date` = ?, `sub_totoal` = ?, `amount_by_date` = ?, `amount_paid` = ?, `term` = ?, `recepients_mails` = ?, `plan_id` = ?, `is_recurring` = ?, `payment_options` = ?, `send_to` = ?,`payment_method` = ?, `tax_amount` = ? WHERE `id` = ?;";
...
//		public final static String UPDATE_INVOICE_AS_PAID_STATE_QRY = "UPDATE invoice SET refrence_number=?,invoice_date=?,payment_method=?,state='paid' WHERE id=?;";
		...
		public final static String GET_UNMAPPED_INVOICES_LIST = "SELECT DISTINCT (i.id), i.`number`, i.`due_date`, i.`amount`, i.`customer_id` AS customerID, c.`customer_name`, i.`amount_due` , i.`state`, i.`currency`, ip.`mapping_id` FROM invoice i LEFT JOIN `invoice_payments_lines` ilp ON ilp.`invoice_id` = i.`id` LEFT JOIN `invoice_payments` ip ON ip.`id` = ilp.`payment_id` LEFT JOIN company_customers c ON c.`customer_id` = i.`customer_id` WHERE i.`company_id`= ? AND  i.`customer_id` = ? AND ip.`mapping_id` IS NULL  ";
	}

...
public class SqlQuerys {
	...
	public final class Invoice {
		public static final String INSERT_QRY = "insert into `invoice` (`id`,`user_id`,`company_id`,`amount`,`currency`,`description`,`objectives`,`last_updated_by`,`last_updated_at`,`state`,`invoice_date`,`acceptance_date`,`acceptance_final_date`,`notes`,`discount`,`deposit_amount`,`processing_fees`,`remainder_json`,`remainder_mail_json`,`is_recurring`,`recurring_frequency`,`recurring_frequency_value`,`recurring_start_date`,`recurring_end_date`,`is_mails_automated`,`is_cc_current_user`,`payment_spring_customer_id`,`po_number`,`document_id`,`amount_due`,`payment_date`,`customer_id`,`sub_totoal`,`amount_by_date`,`created_at`,`amount_paid`,`number`,`created_at_millis`,`term`) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		public static final String UPDATE_QRY = "UPDATE `invoice` SET  `user_id` =?,`company_id` =?,`amount` =?,`currency` =?,`description` =?,`objectives` =?,`last_updated_by` =?,`last_updated_at` =?,`state` =?,`invoice_date` =?,`acceptance_date` =?,`acceptance_final_date` =?,`notes` =?,`discount` =?,`deposit_amount` =?,`processing_fees` =?,`remainder_json` =?,`remainder_mail_json` =?,`is_recurring` =?,`recurring_frequency` =?,`recurring_frequency_value` =?,`recurring_start_date` =?,`recurring_end_date` =?,`is_mails_automated` =?,`is_cc_current_user` =?,`payment_spring_customer_id` =?,`po_number` =?,`document_id` =?,`amount_due` =?,`payment_date` =?,`sub_totoal` =?,`customer_id` =?, `amount_by_date` = ?, `amount_paid` = ?, `number` =?, `term` = ? WHERE `id` =?";
		public final static String DELETE_QRY = "DELETE FROM invoice WHERE `id`=?;";
		public final static String GET_QRY = "SELECT i.`created_at` i_created_at,i.`term` i_term, i.`number` i_number,i.`id`,i.`user_id`,i.`company_id`,i.`amount`,i.`currency`,i.`description`,i.`objectives`,i.`last_updated_by`,i.`last_updated_at`,i.`customer_id`,i.`state`, i.`invoice_date`,i.`acceptance_date`,i.`acceptance_final_date`,i.`notes`,i.`discount`,i.`deposit_amount`,i.`processing_fees`,i.`remainder_json`, i.`remainder_mail_json`,i.`is_recurring`,i.`recurring_frequency`,i.`recurring_frequency_value`,i.`recurring_start_date`,i.`recurring_end_date`, i.`is_mails_automated`,i.`is_cc_current_user`,cc.`payment_spring_id`,i.`po_number`,i.`document_id`,i.`amount_due`,i.`payment_date`,i.`sub_totoal`, i.`amount_by_date`,c.`code`,c.`name`,c.`html_symbol`,c.`java_symbol`, il.`id` AS `ilid`,il.`item_id` AS `il_item_id`,it.`name` AS `il_item_name`,il.`invoice_id`, il.`description` `il_description`,il.`objectives` `il_objectives`,il.`amount` `il_amount`,il.`last_updated_by` `il_last_updated_by`,  il.`last_updated_at` `il_last_updated_at`,il.`quantity` `il_quantity`,il.`price` `il_price`,il.`notes` `il_notes`,il.`coa_id` `il_coa_id`,coa.`name` `il_coa_name`,    ilt.`invoice_line_id` `ilt_invoice_line_id`,ilt.`tax_id` `ilt_tax_id`,ct.`tax_rate` `ilt_tax_rate`,ct.`name` `ilt_name`,cc.`payment_spring_id`,  cc.`customer_name`,cc.`email_id`,comp.`name` `company_name`  FROM `invoice` i    LEFT JOIN `currencies` c ON i.currency=c.code    LEFT JOIN `invoice_lines` il ON i.id=il.invoice_id    LEFT JOIN `invoice_line_taxes` ilt ON il.id=ilt.invoice_line_id    LEFT JOIN `company_taxes` ct ON ilt.tax_id = ct.`id`   LEFT JOIN `company_customers` cc ON cc.`customer_id` = i.`customer_id`  LEFT JOIN `items` it ON il.`item_id` = it.`id`  LEFT JOIN `chart_of_accounts` coa ON il.`coa_id` = coa.`id` LEFT JOIN `company` comp ON i.`company_id` = comp.`id` WHERE i.id  =?;";
		public final static String GET_INVOICES_LIST_QRY = "SELECT `id`,`po_number`,`invoice_date`,`payment_date`,`amount`,`currency`,`last_updated_by`,`last_updated_at`,`customer_id`,`state`,`amount_due`,`sub_totoal`,`amount_by_date` FROM invoice WHERE ";
		public final static String GET_INVOICES_PDF_QRY = "SELECT ip.`default_title`,ip.`template_type`,ip.`default_sub_heading`,  c.`name`,c.`address`,c.`city`,c.`state`,c.`country`,c.`phone_number`,cc.`customer_name`,cc.`email_id`,ip.`standard_memo`,ip.`default_footer` FROM `invoice_preferences` ip LEFT JOIN `company` c ON ip.`company_id` = c.`id` LEFT JOIN `company_customers` cc ON ip.`company_id` = cc.`company_id` WHERE ip.`company_id`= ? AND cc.customer_id=?;";
		public final static String GET_INVOICES_MAIL_QRY = "SELECT cur.`html_symbol`,i.`created_at` i_created_at, i.`number` i_number,cc.`customer_name` cust_name,i.`invoice_date` invoice_date,c.`name` comp_name, i.`currency` i_currency,i.`amount` i_amount,i.`amount_by_date` i_amount_by_date,i.`acceptance_final_date` i_acceptance_final_date,cc.`email_id` cust_email_id FROM invoice i LEFT JOIN `currencies` cur ON i.`currency` = cur.`code` LEFT JOIN company_customers cc ON cc.customer_id = i.customer_id LEFT JOIN company c ON c.id = i.company_id WHERE i.id =? limit 1;";
	...
	public final class InvoicePayment{
		public static final String INSERT_QRY = "INSERT INTO invoice_payments ( `id`,`invoice_id`,`transaction_id`,`amount`,`transaction_date`,`status`,`period_start`,`period_end`) VALUES( ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	public final class Customer {
		public static final String INSERT_QRY = "INSERT INTO  company_customers (`user_id`,`company_id`,`customer_id`,`customer_address`,`customer_city`,`customer_country`,`customer_ein`,`customer_state`,`customer_name`,`customer_zipcode`,`email_id`,`phone_number`,`coa`, `payment_spring_id`,`term`)VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		public static final String UPDATE_QRY = "UPDATE  company_customers SET `customer_address` = ?,`customer_city` = ?,`customer_country` = ?,`customer_ein` = ?,`customer_state` = ?,`customer_name` = ?,`customer_zipcode` = ? , `email_id` = ? , `phone_number` = ?, `user_id`= ?,`company_id` = ?,`coa` = ?, `payment_spring_id` = ?, `term`=?  WHERE `customer_id`= ? ;";
		public static final String DELETE_QRY = "DELETE FROM  company_customers WHERE `user_id`=? AND company_id = ? AND customer_id=?;";
		public static final String RETRIEVE_BY_ID_QRY = "SELECT `user_id`,`company_id`,`customer_id`,`customer_address`,`customer_city`,`customer_country`,`customer_ein`,`customer_state`,`customer_name`,`customer_zipcode`,`email_id`,`phone_number`, `coa`, `term` FROM  company_customers WHERE `customer_id`= ? ";
		public static final String RETRIEVE_LIST_BY_ID_QRY = "SELECT `user_id`,`company_id`,`customer_id`,`customer_address`,`customer_city`,`customer_country`,`customer_ein`,`customer_state`,`customer_name`,`customer_zipcode`,`email_id`,`phone_number`,`coa`, `term` FROM  company_customers WHERE `user_id`= ? AND `company_id` = ?;";
	}
}

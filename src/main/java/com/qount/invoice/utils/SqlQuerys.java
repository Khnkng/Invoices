...
//		public final static String UPDATE_INVOICE_AS_PAID_STATE_QRY = "UPDATE invoice SET refrence_number=?,invoice_date=?,payment_method=?,state='paid' WHERE id=?;";
		...
		public final static String GET_UNMAPPED_INVOICES_LIST = "SELECT DISTINCT (i.id), i.`number`, i.`due_date`, i.`amount`, i.`customer_id`, i.`amount_due` , i.`state`, ip.`mapping_id` FROM invoice i LEFT JOIN `invoice_payments_lines` ilp ON ilp.`invoice_id` = i.`id` LEFT JOIN `invoice_payments` ip ON ip.`id` = ilp.`payment_id`WHERE i.`company_id`= ? AND  i.`customer_id` = ? AND ip.`mapping_id` IS NULL ";
	}

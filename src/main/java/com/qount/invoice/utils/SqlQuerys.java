...
public class SqlQuerys {
	...
	public final class Invoice {
		...
		public final static String GET_QRY = "SELECT  i.`id`, i.`user_id`, i.`company_id`, i.`customer_id`, i.`amount`, i.`currency`, i.`description`, i.`objectives`, i.`last_updated_by`, i.`last_updated_at`, i.`state`, i.`invoice_date`, i.`notes`, i.`discount`, i.`deposit_amount`, i.`processing_fees`, i.`number`, i.`document_id`, i.`amount_due`, i.`payment_date`, i.`sub_totoal`, i.`amount_by_date`, i.`created_at`, i.`amount_paid`, i.`term`, i.`created_at_millis`, i.`recepients_mails`, i.`plan_id`, i.`is_recurring`, i.`payment_options`, i.`email_state`, i.`send_to`, i.`due_date`,il.`id` `il_id`,il.`invoice_id` `il_invoice_id` ,il.`description` `il_description`,il.`objectives` `il_objectives`,il.`amount` `il_amount`,il.`last_updated_by` `il_last_updated_by`,il.`last_updated_at` `il_last_updated_at`,il.`quantity` `il_quantity`,il.`price` `il_price`,il.`notes` `il_notes`,il.`item_id` `il_item_id`,il.`type` `il_type`,il.`tax_id` `il_tax_id`,c.`code`,c.`name`,c.`html_symbol`,c.`java_symbol`,cc.`card_name`,cc.`payment_spring_id`,cc.`customer_name`,it.`name` AS `il_item_name`,coa.`name` `il_coa_name` FROM `invoice` i    LEFT JOIN `currencies` c ON i.currency=c.code    LEFT JOIN `company_customers` cc ON cc.`customer_id` = i.`customer_id`  LEFT JOIN `invoice_lines` il ON i.id=il.invoice_id    LEFT JOIN `items` it ON il.`item_id` = it.`id` LEFT JOIN `chart_of_accounts` coa ON il.`item_id` = coa.`id` WHERE i.id  =?;";
		public final static String GET_INVOICES_LIST_QRY = "SELECT `number`,`id`,`invoice_date`,`payment_date`,`amount`,`currency`,`state`,`amount_by_date`,`due_date`,`amount_due` FROM invoice WHERE ";
	...
	public final class ProposalLine {
		public final static String INSERT_QRY = "INSERT INTO `proposal_lines` (`id`,`proposal_id`,`description`,`objectives`,`amount`,`currency`,`last_updated_by`,`last_updated_at`,`quantity`,`price`,`notes`,`item_id`,`coa_id`,`tax_id`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		public final static String UPADTE_QRY = "UPDATE `proposal_lines` SET `description` = ?,`objectives` = ?,`amount`= ?,`currency` = ?,`last_updated_by`= ?,`last_updated_at` = ?,`quantity` = ?,`price` = ?,`notes` = ?,`item_id` = ?,`coa_id` = ?,`tax_id`=? WHERE `id` = ? AND `proposal_id` = ?;";
		public final static String GET_LINES_QRY = "SELECT `tax_id`,`id`,`proposal_id`,`description`,`objectives`,`amount`,`currency`,`last_updated_by`,`last_updated_at`,`quantity`,`price`,`notes`,`item_id`,`coa_id` FROM proposal_lines WHERE `id` = ?;";
	...
	public final class Payments {
		...
		public static final String RETRIEVE_BY_COMPANYID_QRY = "SELECT * FROM invoice_payments_latest WHERE `company_id` = ?;";
		public static final String RETRIEVE_BY_PAYMENTID_QRY = "SELECT * FROM invoice_payments_latest WHERE `id` = ?;";
	}

...
public class SqlQuerys {
	...
	public final class Payments {
		...
		public static final String RETRIEVE_BY_COMPANYID_QRY = "SELECT invoice_payments.*,journals.`id` AS journal_id,journals.`isActive`,company_customers.`customer_name`, deposits.`id` AS deposit_id FROM invoice_payments LEFT JOIN journals ON `invoice_payments`.id = journals.`sourceID` LEFT JOIN company_customers ON company_customers.`customer_id`= invoice_payments.`received_from` LEFT JOIN transaction_mappings ON transaction_mappings.`id` = invoice_payments.`mapping_id` LEFT JOIN deposits ON deposits.`mapping_id` = invoice_payments.`mapping_id`  WHERE invoice_payments.`company_id`= ?  ORDER BY payment_date DESC  ";
		public static final String RETRIEVE_BY_INVOICE_QRY = "SELECT invoice_payments.*,journals.`id` AS journal_id,journals.`isActive`,company_customers.`customer_name`, deposits.`id` AS deposit_id FROM invoice_payments LEFT JOIN journals ON `invoice_payments`.id = journals.`sourceID` LEFT JOIN company_customers ON company_customers.`customer_id`= invoice_payments.`received_from` LEFT JOIN transaction_mappings ON transaction_mappings.`id` = invoice_payments.`mapping_id` LEFT JOIN deposits ON deposits.`mapping_id` = invoice_payments.`mapping_id` LEFT JOIN invoice_payments_lines  ipl ON ipl.`payment_id` = invoice_payments.`id` WHERE ipl.`invoice_id`= ?  AND ipl.`amount`>0 ORDER BY payment_date DESC  ";
		...
		public static final String RETRIEVE_INVOICES_BY_PAYMENTID = "SELECT `invoice`.`id` AS invoice_id,`invoice`.`number`,`invoice`.`invoice_date`,`invoice`.`due_date`,`invoice`.`amount`,`invoice`.`amount_due`,`invoice`.`state`,`invoice`.`currency`, invoice_payments_lines.id AS payment_lineID,invoice_payments.id AS paymentID ,`company_customers`.`customer_name` FROM invoice_payments LEFT JOIN invoice_payments_lines ON invoice_payments.id = invoice_payments_lines.`payment_id` LEFT JOIN invoice ON invoice_payments_lines.`invoice_id` = invoice.`id` LEFT JOIN company_customers ON invoice.`customer_id` = company_customers.`customer_id` WHERE `invoice_payments`.`id` = ? AND invoice_payments_lines.`amount`>0";
	}
